...
void sFLASH_Erase_32KB_Blk(uint32_t BlkAddr)
{
    ...
    sFLASH_SendByte(sFLASH_CMD_BE_32KB);
    /*!< Send BlkAddr high nibble address byte */
    sFLASH_SendByte((BlkAddr & 0xFF0000) >> 16);
    /*!< Send BlkAddr medium nibble address byte */
    sFLASH_SendByte((BlkAddr & 0xFF00) >> 8);
    /*!< Send BlkAddr low nibble address byte */
    sFLASH_SendByte(BlkAddr & 0xFF);
    /*!< Deselect the FLASH: Chip Select high */
    sFLASH_CS_HIGH();
    /*!< Wait the end of Flash writing */
    sFLASH_WaitForWriteEnd();
...
void sFLASH_Erase_64KB_Blk(uint32_t BlkAddr)
{
    ...
    sFLASH_CS_LOW();
    /*!< Send Block Erase instruction */
    sFLASH_SendByte(sFLASH_CMD_BE_64KB);
    /*!< Send BlkAddr high nibble address byte */
    sFLASH_SendByte((BlkAddr & 0xFF0000) >> 16);
    /*!< Send BlkAddr medium nibble address byte */
    sFLASH_SendByte((BlkAddr & 0xFF00) >> 8);
...
void sFLASH_EraseChip(void)
{
    ...
    sFLASH_SendByte(sFLASH_CMD_BE_CHIP);
    /*!< Deselect the FLASH: Chip Select high */
    sFLASH_CS_HIGH();
    /*!< Wait the end of Flash writing */
    sFLASH_WaitForWriteEnd();
